...
for bond_type in requests.post(PAPER_TYPES_REQUEST).json()["data"].keys():
    all_bonds_by_type: dict = requests.post(PAPER_DATA_REQUEST.format(bond_type)).json()["data"]["data"]
